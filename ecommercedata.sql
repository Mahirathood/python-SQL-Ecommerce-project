use ecommercedata;
#basic query
# 1) list all inuque cities where customers are located
select * from customers;
select distinct customer_city from customers;

#2)  count no.of order placed in 2017
select * from orders;
...
#4) caluclate the percentage of orders that were paid in installments.
...
select * from orders;
select month(order_purchase_timestamp) months, count(order_id) order_id from orders where year(order_purchase_timestamp) = 2018 group by months;
...
select * from payments;
select * from sellers;
select upper(products.product_category) category, round((sum(payments.payment_value)/(select sum(payment_value) from
payments))*100,2) sales_percentage from products join order_items on products.product_id = order_items.product_id
...
on order_items.order_id = payments.order_id group by order_items.seller_id) as a;
...
with a as (select customers.customer_id, min(orders.order_purchase_timestamp) first_order from customers join orders
...
select years, customer_id, payment, d_rank from (select year(orders.order_purchase_timestamp) years,
